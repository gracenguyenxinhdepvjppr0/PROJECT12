    ...
    try:
        ...
        if line.startswith('---'):
            ...
            if len(frame_data) == 768:
                ...
                # (Bạn có thể sửa T_MAX = 50.0 nếu đo vật nóng hơn)
                T_MIN = 20.0
                T_MAX = 40.0
                temp_array = np.clip(temp_array, T_MIN, T_MAX)

                # 3. Chuẩn hóa về 0-255 và làm mượt ảnh (phóng to lên 640x480)
                ...
                cv2.imshow('MLX90640 Heatmap', heatmap)
            ...
            # Cắt chuỗi bằng khoảng trắng (space) và thêm vào mảng frame_data
            values = [float(x) for x in line.split() if x.strip()]
            frame_data.extend(values)

    except ValueError:
        # Bỏ qua nếu có chữ cái lọt vào (ví dụ chữ "Loi doc EEPROM!")
        pass
    except UnicodeDecodeError:
        # Bỏ qua nhiễu tín hiệu UART
        pass
