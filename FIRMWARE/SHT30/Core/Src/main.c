...
int main(void)
{
  ...
  {
    /* USER CODE END WHILE */
	  if (SHT30_Read(&temperature, &humidity) == HAL_OK)
	  	      {
	  		  //SH1106_Write("Temp: ", &Font_7x10, 5, 5);
	  		  sprintf(buffer, "Temp: %.2f Humid: %.2f \r\n", temperature, humidity);
	  		  HAL_UART_Transmit(&huart1, (uint8_t*)buffer, strlen(buffer), HAL_MAX_DELAY);
//	  		  //SH1106_Write(bufftemp, &Font_7x10, 47, 5);
//	  		  sprintf(buffhumid, "Humid: %.2f\r\n", humidity);
//	  		  HAL_UART_Transmit(&huart1, (uint8_t*)buffhumid, 50, HAL_MAX_DELAY);
	  		  //		  sprintf(buffertemp, "Temp: %.2f", temperature);
	  //		  HAL_UART_Transmit(&huart1, (uint8_t*)buffertemp, 50, HAL_MAX_DELAY);
	  //		  HAL_Delay(10);
	  //		  sprintf(bufferhumid, "Humid: %.2f", humidity);
	  //		  HAL_UART_Transmit(&huart1, (uint8_t*)bufferhumid, 50, HAL_MAX_DELAY);
	  //		  SH1106_GotoXY(10,5);
